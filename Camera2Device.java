...
public class Camera2Device implements ICamera {
    ...
    // 预览视图
    private CameraPreview mPreviewTexture;
    //相机管理器
...
//    private Size mCaptureSize;
    ...
    @Override
    public void configPreviewSize(CameraPreview textureView, int width, int height) {
...
//            }
    ...
    private ImageReader.OnImageAvailableListener onImageAvailableListener = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            TipsUtil.log("===onImageAvailable===");
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), new File("/mnt/sdcard/" + simpleDateFormat.format(new Date()) + ".jpg")));
//
...
//============================2018-11-02========================//
/**
 * Created by zhang.huajian2 on 2018/10/11.
 * 5.0以上的相机设备
 */
@RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
public class Camera2Device implements ICamera {
    ...
    private int mCurrentFacing = CameraCharacteristics.LENS_FACING_FRONT;
    // 预览视图
    private BaseCameraPreview mPreviewTexture;
    //相机管理器
    private CameraManager mCameraManager;
    private CameraCharacteristics cameraCharacteristics;
    // 相机最大的预览宽度
...
//    private Size mCaptureSize;
    ...
    @Override
    public void configPreviewSize(BaseCameraPreview textureView, int width, int height) {
        this.mPreviewTexture = textureView;
        mCameraManager = (CameraManager) context.getSystemService(Context.CAMERA_SERVICE);
        // 开启后台工作线程
        startWorkThread();
...
//                TipsUtil.log((facing == CameraCharacteristics.LENS_FACING_BACK ? "后" : "前") + " 摄像头 " + " 是否支持自动对焦 " + mAutoFocusSupported + " 获取到焦距的最大值 " + minimum_focus_distance + " 最大的缩放值 " + maxZoom);
                ...
                mPreviewSize = chooseOptimalSize(map.getOutputSizes(SurfaceTexture.class),
                        ...
                        maxPreviewHeight, new CompareSizesByArea());
                //--2018/10/11 先使用预览大小---对于静态图片，使用可用的最大值来拍摄。
//                mCaptureSize = Collections.max(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)), new CompareSizesByArea());
                //设置ImageReader,将大小，图片格式
                mImageReader = ImageReader.newInstance(mPreviewSize.getWidth(), mPreviewSize.getHeight(), ImageFormat.JPEG, 2);
                mImageReader.setOnImageAvailableListener(onImageAvailableListener, mBackgroundHandler);
//                // 计算出来的预览大小，设置成TextureView宽高.
...
//            }
    ...
    private class ImageSaver implements Runnable {
        ...
        public void run() {
            ...
            uiHandler.sendEmptyMessage(1);
            //保存图像位置
//            saveLocation(mFile.getPath(),lat,lng);
        }
...
//            mMediaRecorder.setVideoSize(640, 480);
            ...
            //记录视频位置
//            mMediaRecorder.setLocation(lat,lng);
            // 设置最大录制时间(默认2min)
            mMediaRecorder.setMaxDuration(2*60*10000);
            //2018-10-22先试试自定义配置 //配置录制的视频为720p
//            mMediaRecorder.setProfile(CamcorderProfile.get(mCurrentFacing, CamcorderProfile.QUALITY_720P));
            int rotation = ((WindowManager) context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getRotation();
            switch (mSensorOrientation) {
    ...
    private MediaRecorder.OnInfoListener mediaRecorderInfoListener = new MediaRecorder.OnInfoListener() {
        @Override
